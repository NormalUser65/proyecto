...
export function DetalleCategoria() {
  const { t } = useTranslation("detalleCategoria");
  ...
  if (error) return <ErrorAlert title={t("detalleCategoria.errorTitulo")} message={error}/>;
  if (!categoria || categoria.data.length === 0)
    return <EmptyState message={t("detalleCategoria.emptyMessage")} />;
    ...
    <div className="max-w-4xl mx-auto py-12 px-4">
      ...
      <Card className={" rounded-2xl !shadow-[var(--box-shadow)]"}>
        <CardContent className="p-6 space-y-4">
          <div>
            <span className="font-semibold">{t("detalleCategoria.descripcion")}</span>
            <p className="text-muted-foreground">{data.Descripcion}</p>
          </div>
          <div className="flex items-center gap-4">
            <Clock className="h-5 w-5 text-secondary" />
            <span className="font-semibold">{t("detalleCategoria.sla")}</span>
            <p className="text-muted-foreground">
              {data.SLA} ({data.Tiempo_Max_Respuesta} min resp.,{" "}
              {data.Tiempo_Max_Resolucion} min resolución)
            </p>
          </div>
          {data.Etiquetas && (
            <div>
          ...
          {data.Especialidades && (
            <div>
              <span className="font-semibold flex items-center gap-2">
                <User className="h-5 w-5 text-secondary" />{t("detalleCategoria.especialidades")}</span>
      ...
      >
        ...
        {t("detalleCategoria.botonRegresar")}
      </Button>
    </div>
  );
}